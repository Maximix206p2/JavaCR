...
Сколько персонажей разных рас
select c.name, count(c.race_id)
from character as c
inner join race as r
on (c.race_id=r.id)
group by r.name;
...
where name Like'T%'and race_id=2 ;
...
select c.name, r.name, l.name
from character as c
inner join race as r, class as l
on (c.race_id=r.id)
where (r.name='Humans') and (l.name='Archer');
