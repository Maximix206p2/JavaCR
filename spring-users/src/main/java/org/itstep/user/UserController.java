...
public class UserController {
    ...
    public String updateUser(User user, Model model, @RequestParam(value = "role" , required = false) long[] roles) {
        Set<Role> rolesNew = new HashSet<>();
        if(roles != null) {
    ...
    public String createUser(User user, Model model, @RequestParam(value = "role" , required = false) long[] roles) {
        Set<Role> rolesNew = new HashSet<>();
        if(roles != null) {
            for (int i = 0; i < roles.length; i++) {
                //System.out.println(roles[i]);
        ...
        User userDb = userService.findById(user.getId());
        userDb.setUsername(user.getUsername()); //ещё пароль нужен
        userDb.setEnabled(user.isEnabled());
        userDb.setRoles(rolesNew);
        userService.save(userDb);
        model.addAttribute("user", userService.findAll());
